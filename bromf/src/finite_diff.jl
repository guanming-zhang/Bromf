...
#         diff_coeff = [-1.0/12.0  0.0  1.0/12.0;
#                       -1.0/3.0   0.0  1.0/3.0;
#                       -1.0/12.0  0.0  1.0/12.0]
#     else
#         error("mode = central or forward, no other schemes implemented")
#     end
#     nrow,ncol = size(diff_coeff)
#     nbrs = ncol ÷ 2         # integer division
#     diff_mat = spzeros(Float64, N, N)
...
#     if mode == "isotropic"
#         nbrs = 1
#         stencil_ind =  Array{Int64,2}(undef,nrow,ncol)
#     end

#     #note that x direction is the downwards direction and y-direction is the right direction
#     #note that the finite difference code is written keeping in mind that a 2D matrix is flattened along columns into a 1D array (also the default way to do via reshape in julia)
#     for i in 1:nx
#         for j in 1:ny   
#             if along == 1 
#                 if mode == "isotropic"
#                     stencil_ind[2,2] = i + (j-1)*nx     

#                     stencil_ind[1,1] = mod_idx(i-1,nx) + (mod_idx(j-1,ny)-1)*nx
#                     stencil_ind[2,1] = mod_idx(i-1,nx) + (mod_idx(j,ny)-1)*nx
#                     stencil_ind[3,1] = mod_idx(i-1,nx) + (mod_idx(j+1,ny)-1)*nx

#                     stencil_ind[1,3] = mod_idx(i+1,nx) + (mod_idx(j-1,ny)-1)*nx 
#                     stencil_ind[2,3] = mod_idx(i+1,nx) + (mod_idx(j,ny)-1)*nx 
#                     stencil_ind[3,3] = mod_idx(i+1,nx) + (mod_idx(j+1,ny)-1)*nx 

#                 elseif mode == "central" || mode == "forward"
#                     stencil_ind[nbrs + 1] = i + (j-1)*nx                             # center
#                     for nl in 1:nbrs
#                         stencil_ind[nbrs + 1 - nl] = mod_idx(i - nl, nx) + (j-1)*nx   # the point left to the center by nl unit
#                     end
...
#                     stencil_ind[1,1] = mod_idx(i-1,nx) + (mod_idx(j-1,ny)-1)*nx 
#                     stencil_ind[2,1] = mod_idx(i,nx) + (mod_idx(j-1,ny)-1)*nx 
#                     stencil_ind[3,1] = mod_idx(i+1,nx) + (mod_idx(j-1,ny)-1)*nx 
                    
#                     stencil_ind[1,3] = mod_idx(i-1,nx) + (mod_idx(j+1,ny)-1)*nx 
#                     stencil_ind[2,3] = mod_idx(i,nx) + (mod_idx(j+1,ny)-1)*nx 
#                     stencil_ind[3,3] = mod_idx(i+1,nx) + (mod_idx(j+1,ny)-1)*nx 
...
#                     for js in 1:ncol
#                         if js != 2
#                             diff_mat[stencil_ind[2,2],stencil_ind[is,js]] = diff_coeff[is,js]
#                         end
#                     end
#                 end
#             end
...
"""
    ...
    for ix in 1:2*Kx+1
        for iy in 1:2*Ky+1
            ...
            #reminder - center of the kernel is (Kx+1,Ky+1)
            _i = mod_idx(x[1]+Kx+1-ix,Fx)
            _j = mod_idx(x[2]+Ky+1-iy,Fy)
...
function convol2d_fft(f::Array{Float64,2},k::Array{Float64,2},dx,dy,p,p_i)
    f = complex(f)
    k = complex(k)
    a_temp = similar(f)
    b_temp = similar(f)
    ...
    mul!(d_temp, p_i, c_temp)

    return real(d_temp).*dx.*dy
end

# #convolution function using FFT
# function convol2d_fft(f::Array{Float64,2},k::Array{Float64,2},dx,dy)
#     f = complex(f)
#     k = complex(k)
#     return real(ifft!(fft!(f).*fft!(k))).*dx.*dy
# end
